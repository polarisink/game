...
import static github.polarisink.vgq.infrastructure.asserts.BusinessE.JACKSON_ERROR;
import static github.polarisink.vgq.infrastructure.utils.TimeUtil.SF;


/**
 * jackson工具类
 * TODO rewrite exception log
 *
 * @author lqs
 * @date 2022/3/19
...
public class JacksonUtil {
  ...
  public static ObjectMapper initMapperConfig(ObjectMapper objectMapper) {
    objectMapper.setDateFormat(new SimpleDateFormat(TimeUtil.S_F_STR));
    //配置序列化级别
    ...
    javaTimeModule.addSerializer(LocalDateTime.class, new LocalDateTimeSerializer(SF))
        .addDeserializer(LocalDateTime.class, new LocalDateTimeDeserializer(SF));
  ...
  public static <V> V from(URL url, Class<V> type) {
    try {
      ...
      e.printStackTrace();
      throw new BaseException(JACKSON_ERROR);
    }
  }
